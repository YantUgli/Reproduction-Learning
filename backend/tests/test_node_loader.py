...
def test_load_real_nodes_full(session: Session):
    ...
    assert report.instances >= 10  # >=2 varian/node
    assert report.probes >= 5  # >=1 probe/node
...
def test_load_is_idempotent(session: Session):
    ...
    # Jalan dua kali tak menggandakan baris.
    assert len(session.exec(select(Node)).all()) == 5
    assert len(session.exec(select(Edge)).all()) == 4
    n_instances = len(session.exec(select(ChallengeInstance)).all())
    assert n_instances == r1.instances
